...
CREATE TABLE profiles (
    ...
    luteal_phase_length INTEGER DEFAULT 14 CHECK (luteal_phase_length BETWEEN 10 AND 16),
...
CREATE TABLE check_ins (
    ...
    -- Dados do check-in
    free_text TEXT,
    humor_emoji TEXT CHECK (humor_emoji IN ('bad', 'low', 'neutral', 'good', 'great')),
...
CREATE TABLE weekly_learnings (
    ...
    -- Padrões aprendidos
    phase TEXT,
    success_rate DECIMAL(5,2), -- Percentual de sucesso (0-100)
...
CREATE INDEX idx_tasks_user_due_date ON tasks(user_id, due_date, is_completed);
CREATE INDEX idx_tasks_user_completed ON tasks(user_id, is_completed);
CREATE INDEX idx_cycle_data_user_period ON cycle_data(user_id, period_start_date DESC);
CREATE INDEX idx_weekly_learnings_user_week ON weekly_learnings(user_id, week_start DESC);
...
CREATE POLICY "Users can manage own cycle_data"
    ON cycle_data FOR ALL
    USING (auth.uid() = user_id);
...
CREATE POLICY "Users can manage own weekly_learnings"
    ON weekly_learnings FOR ALL
    USING (auth.uid() = user_id);
